...
# --------------------------------------------------
I = -I. -I$(SRCDIR) -Ilib/libinsight-3.2.6/include/mdc_gateway_client/include -Ilib/libinsight-3.2.6/protobuf
L = -L$(BINDIR) -Llib/libinsight-3.2.6/lib
...
# --------------------------------------------------
# bin: huatai_insight_feeder
# --------------------------------------------------
SRC_INSIGHT_FEEDER := $(wildcard src/insight_feeder/*.cc)
BIN_INSIGHT_FEEDER  = huatai_insight_feeder
LIB_INSIGHT_FEEDER = -lprotobuf -lprotobuf-lite -lfeeder -lmdc_gateway_client -lACE_SSL -lACE -lssl $(LIBS_COMMON)

OBJ_INSIGHT_FEEDER := $(SRC_INSIGHT_FEEDER:%.cc=%.o)
$(BIN_INSIGHT_FEEDER): $(OBJ_INSIGHT_FEEDER)
	@echo ">>> build $(BIN_INSIGHT_FEEDER) ..."
	-@mkdir -p bin >/dev/null 2>&1
	-@cp -rf lib/libinsight-3.2.6/lib/*.so* bin/ >/dev/null 2>&1
	...
	-@cd bin; ln -s libprotobuf.so.32 libprotobuf.so >/dev/null 2>&1
